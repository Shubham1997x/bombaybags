...
export interface AdminTaxonomy {
  collections: Array<{ id: number; name: string; image: string | null; productCount: number }>;
  categories: Array<{
    id: number;
    name: string;
    collectionId: number;
...
export async function adminGetTaxonomy(): Promise<AdminTaxonomy> {
  await requireAdmin();
  const [collections, categories, subcategories, tags] = await Promise.all([
    ...
    prisma.category.findMany({
      orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
      include: { collection: { select: { name: true } }, _count: { select: { products: true } } },
    }),
...
export async function adminCreateCollection(input: { name: string; image?: string | null }) {
  await requireAdmin();
...
export async function adminRenameCollection(input: { id: number; name: string; image?: string | null }) {
  await requireAdmin();
  const data = idSchema
    .merge(nameSchema)
    .extend({ image: z.string().trim().max(300).nullable().optional() })
...
export async function adminSetCollectionImage(input: { id: number; image: string | null }) {
  await requireAdmin();
  const data = idSchema.extend({ image: z.string().trim().max(300).nullable() }).parse(input);
  await prisma.collection.update({ where: { id: data.id }, data: { image: data.image } });
  return { ok: true as const };
}

export async function adminDeleteCollection(input: { id: number }) {
  await requireAdmin();
  const data = idSchema.parse(input);
  const inUse = await prisma.category.count({ where: { collectionId: data.id } });
  if (inUse > 0) return { ok: false as const, error: "in_use" };
  await prisma.collection.delete({ where: { id: data.id } });
  return { ok: true as const };
}

export async function adminCreateCategory(input: { name: string; collectionId: number }) {
  await requireAdmin();
  const data = nameSchema.extend({ collectionId: z.number().int().positive() }).parse(input);
  try {
...
export async function adminRenameCategory(input: { id: number; name: string; collectionId?: number }) {
  await requireAdmin();
  const data = idSchema
    .merge(nameSchema)
    .extend({ collectionId: z.number().int().positive().optional() })
    .parse(input);
  try {
    await prisma.category.update({
      where: { id: data.id },
      data: { name: data.name, slug: slugify(data.name), collectionId: data.collectionId },
    });
    return { ok: true as const };
  } catch (err) {
    if (isUniqueViolation(err)) return { ok: false as const, error: "duplicate" };
