...
const overlayBtn =
  "flex items-center justify-center rounded-full bg-paper/90 text-ink shadow-[0_1px_3px_rgba(16,20,43,0.2)] transition-colors hover:bg-ultra hover:text-paper focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ultra/40";
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 pb-16 pt-20 md:px-8 md:pb-24 md:pt-24">
      <button
        type="button"
        onClick={() => router.back()}
        className="mb-3 inline-flex items-center gap-1.5 text-[13px] font-semibold text-ink-soft transition-colors hover:text-ultra focus-visible:outline-none focus-visible:text-ultra"
      >
      ...
      <nav className="mb-5 flex flex-wrap items-center gap-1.5 font-mono text-[12px] uppercase tracking-[0.08em] text-ink-soft">
        <Link href="/" className="transition-colors hover:text-ultra focus-visible:outline-none focus-visible:text-ultra">
          Collections
        </Link>
        <span className="text-stitch">/</span>
        <Link href={`/${product.collectionSlug}`} className="transition-colors hover:text-ultra focus-visible:outline-none focus-visible:text-ultra">
          {product.collectionName}
        </Link>
        <span className="text-stitch">/</span>
        <Link
          href={`/${product.collectionSlug}/${product.categorySlug}`}
          className="transition-colors hover:text-ultra focus-visible:outline-none focus-visible:text-ultra"
        >
      ...
      <div className="grid gap-8 md:grid-cols-2 md:gap-10 lg:gap-14">
        ...
        <div className="md:sticky md:top-24 md:self-start">
          <div className="relative aspect-square overflow-hidden rounded-none border border-hairline bg-tint">
            {images[active] ? (
              <img
                src={images[active]}
            ...
            <div className="mt-3 flex gap-2.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {images.map((url, i) => (
                <button
                  ...
                  className={`h-16 w-16 shrink-0 overflow-hidden rounded-none border-2 bg-tint transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ultra/40 ${
        ...
        <div className="flex flex-col">
          {product.subcategoryName && (
            <span className="font-mono text-[11px] uppercase tracking-[0.14em] text-ultra">
              {product.subcategoryName}
            </span>
          )}
          <h1 className="mt-1.5 font-display text-2xl font-semibold tracking-tight text-ink md:text-3xl">
            {product.name}
          </h1>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-none px-5 py-3.5 text-sm font-semibold transition-colors duration-300 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ultra/40 ${
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex flex-1 items-center justify-center gap-2 rounded-none bg-green px-5 py-3.5 text-sm font-semibold text-white transition-[filter] duration-300 hover:brightness-110 active:brightness-95"
            >
          ...
          <button
            ...
            className="mt-3 flex items-center justify-center gap-2 rounded-none border border-hairline px-5 py-3.5 text-sm font-semibold text-ink transition-colors duration-300 hover:border-ultra hover:text-ultra active:bg-tint focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ultra/40"
          >
            Send Inquiry
            <svg viewBox="0 0 16 16" fill="none" className="h-4 w-4 shrink-0" aria-hidden="true">
          ...
          <div className="mt-8">
            ...
            <h2 className="mt-6 font-mono text-sm font-semibold uppercase tracking-[0.12em] text-ink">
              Product details
            </h2>
          ...
          {product.tags.length > 0 && (
            <ul className="mt-6 flex flex-wrap gap-1.5">
              {product.tags.map((tag) => (
                <li
                  key={tag}
                  className="rounded-none bg-tint px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.06em] text-ultra"
                >
                  {tag}
                </li>
              ))}
