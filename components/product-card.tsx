...
export function ProductCard({
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-none border border-hairline bg-paper transition-all duration-300 hover:-translate-y-0.5 hover:dashed-stitch">
      <Link
      ...
      >
        <div className="relative aspect-square w-full overflow-hidden bg-tint">
          {product.subcategoryName && (
            <span className="absolute left-2.5 top-2.5 z-10 rounded-none bg-paper/90 px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.12em] text-ultra shadow-sm backdrop-blur-sm">
              {product.subcategoryName}
            </span>
          )}
          {product.image ? (
          ...
          {product.tags.length > 0 && (
            <ul className="mt-2 flex flex-wrap gap-1.5">
              {product.tags.slice(0, 2).map((tag) => (
                <li
                  key={tag}
                  className="rounded-none bg-tint px-2 py-0.5 font-mono text-[11px] uppercase tracking-[0.06em] text-ultra"
                >
                  {tag}
                </li>
              ))}
            ...
            <p className="text-lg font-bold tabular-nums text-ink">
              {product.price != null ? (
                <>
                  ...
                  <span className="ml-1 font-mono text-xs font-medium text-ink-soft">
                    /{product.priceUnit}
                  </span>
                </>
              ) : (
      ...
      <div className="border-t border-hairline p-3 pt-2.5 md:p-4 md:pt-3">
        <button
          ...
          className={`flex w-full items-center justify-center gap-2 rounded-none px-3 py-2.5 text-[13px] font-semibold transition-colors duration-300 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ultra/40 ${
            inCart
      ...
      <div className="grid grid-cols-2 divide-x divide-hairline border-t border-hairline">
        <a
          ...
          className="flex items-center justify-center gap-1.5 px-2 py-2.5 font-mono text-[11px] uppercase tracking-[0.1em] text-ink-soft transition-colors duration-300 hover:bg-tint hover:text-green"
        >
        ...
        <button
          ...
          className="flex items-center justify-center gap-1.5 bg-paper px-2 py-2.5 font-mono text-[11px] uppercase tracking-[0.1em] text-ink-soft transition-colors duration-300 hover:bg-tint hover:text-ultra"
        >
          Send Inquiry
        </button>
      </div>
